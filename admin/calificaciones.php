...
<main class="app-content">
    <div class="app-title">
        <div>
          <h1><i class="fa fa-dashboard"></i> Gestión Calificaciones  
          <button class="btn btn-primary" data-toggle="modal" data-target="#registro">Registrar Calificación </button>
        </h1>
        </div>
    </div>

    <div class="tile">
            <h3 class="tile-title">Calificaciones</h3>
            <div class="tile">
            <h3 class="tile-title">Generar reporte por estudiante</h3>
            <div class="tile-body">
            
              <form class="row" method="POST" action="../../reportes/boletines/calificaciones.php">
              <div class="form-group col-md-3">
                  <label class="control-label">Seleccione el Id</label>
                  <select class="form-control" id="exampleSelect1" name="calificacion">
                    <option>1</option>
                    <option>2</option>
                    <option>3</option>
                    <option>4</option>
         ...
         <table class="table table-bordered">
              ...
              <tbody>
                <?php
                $conexion = $db->connect();
                $query = $conexion->prepare('SELECT * FROM `calificaciones`');
                $query->execute();
